...
Процедура ПриОткрытии(Отказ)
	...
	МассивПроверок.Добавить(Новый Структура("ИмяПроцедуры,Описание",
	...
	МассивПроверок.Добавить(Новый Структура("ИмяПроцедуры,Описание",
		...
		"Контроль наличия повторяющегося кода"));
...
Процедура ИнициализироватьПараметрыФормы()
	...
	ПредельноеКоличествоВременныхТаблиц = 10;
...
Процедура ОбновитьСтруктуруМодуля()
	...
	Для Каждого ТекСтрока Из СтруктураМодуля.ПолучитьЭлементы() Цикл
		Если Не СтрНачинаетсяС(ТекСтрока.Директива, "НаКлиенте") Тогда
...
Процедура ОбновитьСтруктуруМодуляРекурсивно(ТекущаяВетка,НомерСтроки)
	...
	Пока НомерСтроки <> ТекстМодуля.КоличествоСтрок() И Не СекцияЗакончилась Цикл
		...
		Если ЭтоНачалоКонецОператорныхСкобок(НомерСтроки, ОписаниеСкобок) Тогда
...
Процедура КонтрольНаличияПовторяющегосяКода()
	...
	Для НомерСтроки = 2 По КоличествоСтрок Цикл
		...
		ПредыдущаяСтрокаКода = ПолучитьЗаполненнуюСтроку(НомерСтроки, -1, НомерНачальнойСтроки);
		
		Если НомерСтроки <> КоличествоСтрок Тогда
			СледующаяСтрокаКода = ПолучитьЗаполненнуюСтроку(НомерСтроки, 1);
		Иначе
			СледующаяСтрокаКода = "";
		КонецЕсли;	
		
		Если СтрСравнить(СтрокаКода, ПредыдущаяСтрокаКода) = 0 И СтрДлина(СтрокаКода) > 0 Тогда
			Если КоличествоПовторов=0 Тогда
				НачальнаяСтрока = НомерНачальнойСтроки;
			КонецЕсли;
		...
		Если КоличествоПовторов > 0 И СтрСравнить(СтрокаКода, СледующаяСтрокаКода) <> 0 Тогда
			
			КоличествоПовторов = 0;
			
			ОшибокНеВыявлено = Ложь;
			ВывестиСообщение("Используется повторяющийся код", НачальнаяСтрока, НомерСтроки);
