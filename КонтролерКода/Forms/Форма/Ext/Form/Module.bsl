...
Процедура ПриОткрытии(Отказ)
	...
	МассивПроверок.Добавить(Новый Структура("ИмяПроцедуры,Описание,ТребуетсяАнализ,РаботаИзЛюбойБазы",
		"КонтрольНаСоединениеВиртуальныхТаблицСФизическими",
		СтрШаблон("Соединение c виртуальными таблицами")));
...
Процедура ПодборатьИзGitHub(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПодборатьИзGitHubЗавершение", ЭтаФорма);
	ПараметрыФормы = Новый Структура("АдресМассиваФайловGitHub",АдресМассиваФайловGitHub);
	ОткрытьФорму("ВнешняяОбработка.КонтролерКода.Форма.ПодборИзGitHub",ПараметрыФормы,,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ОчиститьСтрокуОтКомментария(СтрокаКода)
	...
	Для ИндексСтроки = 1 По СтрЧислоСтрок(РазбитаяСтрока) Цикл
		...
		Если СтрЗаканчиваетсяНа(Подстрока,"""") Тогда
			Продолжить;
		Иначе
			// дальше комментарий
			Прервать;	
...
Процедура ВыполнитьПроверкуПоСценарию() Экспорт
	...
	Если Не КаталогНаДиске.Существует() Тогда
		ВызватьИсключение СтрШаблон("В качестве параметра запуска нужно указать существующий и доступный каталог. Текущий параметр: %1",
			ПараметрЗапуска);
	КонецЕсли;
...
Процедура ПроверитьЗапросыВКодеНаСервере()	
	...
	Если ЗапросыПроанализированы Тогда
		Возврат;
	КонецЕсли;
	...
	ЗапросыПроанализированы = Истина;
...
Процедура ПроанализироватьТекстЗапроса(ТекстЗапроса, ИсходнаяСтрока)
	    СтруктураБазыДанных = ПолучитьСтруктуруХраненияБазыДанных();
		...
		Отбор = Новый Структура;
		Отбор.Вставить("ИмяПроцедуры", "КонтрольНаСоединениеВиртуальныхТаблицСФизическими");
		Отбор.Вставить("Выбор", Истина);
		Если Проверки.НайтиСтроки(Отбор).Количество() > 0 Тогда
			КонтрольНаСоединениеВиртуальныхТаблицСФизическими = Истина;	
		Иначе
			КонтрольНаСоединениеВиртуальныхТаблицСФизическими = Ложь;
		КонецЕсли;
		...
		Для каждого Пакет из СхемаЗапроса.ПакетЗапросов Цикл
			...
			Для каждого Оператор из Пакет.Операторы Цикл
				...
				Для каждого Источник из Оператор.Источники Цикл
					Если КонтрольНаСоединениеВиртуальныхТаблицСФизическими Тогда
						ПроанализироватьСоединения(Источник, ИсходнаяСтрока);	
					КонецЕсли;
				КонецЦикла;
...
Процедура ПроанализироватьПолеЗапроса(ПолеЗапроса, Оператор, ИсходнаяСтрока)
	...
	Для каждого ПутьКДанным из НайденныеПоляСТочками Цикл
		...
		Адрес = ЗаменитьТекст(ПутьКДанным,ИсточникТекст[0] + "\." , "", Ложь);
...
&НаКлиенте
Процедура КонтрольНаСоединениеВиртуальныхТаблицСФизическими()
	ПроверитьЗапросыВКодеНаСервере();
КонецПроцедуры
...
&НаСервере
Процедура ПроанализироватьСоединения(Источник, ИсходныйТекст)
	
	Если ТипЗнч(Источник.Источник) = Тип("ВложенныйЗапросСхемыЗапроса") ИЛИ СтрЧислоВхождений(Источник.Источник.ИмяТаблицы, ".") > 1  Тогда
		
		Для каждого Соединение из Источник.Соединения Цикл
			НоваяСтрока = Сообщения.Добавить();
			НоваяСтрока.Сообщение = "Соединение с виртуальной таблицей";
			НоваяСтрока.НомерСтроки = НайтиСтрокуВТекстовомДокументе(Соединение.Условие);
		КонецЦикла;
	...
	Для каждого Соединение из Источник.Соединения Цикл
		...
		Если ТипЗнч(Соединение.Источник.Источник) = Тип("ВложенныйЗапросСхемыЗапроса") ИЛИ СтрЧислоВхождений(Соединение.Источник.Источник.ИмяТаблицы, ".") > 1 Тогда
			НоваяСтрока = Сообщения.Добавить();
			НоваяСтрока.Сообщение = "Соединение с виртуальной таблицей";
			НоваяСтрока.НомерСтроки = НайтиСтрокуВТекстовомДокументе(Соединение.Условие);			
		КонецЕсли;
